...
<div class="tls-page-header">
    ...
    <div class="tls-top-actions d-flex gap-2">
        <!-- New Driver Button (always visible) -->
...
<div class="tls-form-card">
    <div class="card-header">
        <h5 class="card-title mb-0">
            <i class="bi-search me-2"></i>Driver Search
        </h5>
    </div>
    <div class="card-body">
        <form method="POST" action="<?= base_url('safety/driver-maintenance/search') ?>" id="searchForm">
            <?= csrf_field() ?>

            <!-- Row 1: Label -->
            <div class="row">
                <div class="col-md-8">
                    <label for="driver_key" class="form-label">Search Driver:</label>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="position-relative">
                        <input type="text" class="form-control" id="driver_key" name="driver_key"
...
<?php if ($driver): ?>
<!-- Driver Form -->
<form method="POST" action="<?= base_url('safety/driver-maintenance/save') ?>" id="driver-form">
    <?= csrf_field() ?>
    <input type="hidden" name="driver_key" value="<?= esc($driver['DriverKey'] ?? 0) ?>">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control readonly-field" id="driver_key_display"
                                   value="<?= esc($driver['DriverKey'] ?? '') ?>" readonly>
                        </div>
                        <div class="col-md-9">
                            <label for="driver_name" class="form-label">Driver Name <span class="text-danger">*</span>:</label>
                            <input type="text" class="form-control" id="driver_name" name="name"
                                   value="<?= esc($driver['NAME'] ?? '') ?>" required>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-4">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" id="division" name="division">
                                <?php for ($i = 1; $i <= 7; $i++): ?>
                                <option value="<?= $i ?>" <?= ($driver['Division'] ?? 1) == $i ? 'selected' : '' ?>>
                                    Division <?= $i ?>
                                </option>
                                <?php endfor; ?>
                            </select>
                        ...
                        <div class="col-md-12">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="full_freight_pay" name="full_freight_pay"
                                       <?= ($driver['FullFreightPay'] ?? 0) ? 'checked' : '' ?>>
                ...
                <div class="card-body">
                    ...
                    <div id="tax_id_section" style="display: none;">
                        <div class="mb-3">
                            ...
                            $defaultType = 'O';
                            if ($driver && isset($driver['IDType']) && trim($driver['IDType']) !== '') {
                                $defaultType = trim($driver['IDType']);
                            }
                            ?>
...
<script>
    ...
    const agentSearchField = document.getElementById('driver_key');
    ...
    const agentAutocomplete = new TLSAutocomplete(
        ...
        'drivers',
        function(agent) {
            console.log('Driver selected:', agent);
    ...
    // Handle form submission - allow direct DriverKey entry or search
    document.getElementById('searchForm').addEventListener('submit', function(e) {
        ...
        if (!searchValue) {
            e.preventDefault();
            alert('Please enter an DriverKey or select an agent from the search results.');
            return false;
        }
    ...
    function newDriver() {
        ...
        // Confirm before creating new agent (reserves DriverKey)
        if (!confirm('Are you sure you want to create a new agent?\n\nThis will reserve an Driver Key in the database.')) {
            return;
        }

        // Make AJAX call to create the agent and get DriverKey
        ...
        .then(data => {
            if (data.success && data.driver_key) {
    ...
    function loadDriverAddress() {
        // Get the hidden driver_key field from the agent form (not the search form)
        const driverKeyField = document.querySelector('#driver-form input[name="driver_key"]');
        ...
        const driverKey = driverKeyField.value;
        console.log('Loading address for driver:', driverKey);

        if (!driverKey || driverKey == '0') {
            document.getElementById('address-display').innerHTML = '<p class="text-muted"><em>No agent loaded</em></p>';
            return;
        }

        const url = `<?= base_url('safety/driver-maintenance/get-address') ?>?driver_key=${driverKey}`;
    ...
    function editAddress() {
        if (!currentAddress) {
            alert('Please load an driver first');
            return;
        }
    ...
    function saveAddress() {
        // Get the hidden driver_key field from the agent form (not the search form)
        const driverKey = document.querySelector('#driver-form input[name="driver_key"]').value;
        ...
        formData.append('driver_key', driverKey);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        applyInputMask();
        loadDriverAddress(); // Load address when page loads
        loadDriverContacts(); // Load contacts when page loads
        loadDriverComments(); // Load comments when page loads
    });
    ...
    function loadDriverContacts() {
        ...
        const driverKey = driverKeyField.value;
        console.log('Loading contacts for driver:', driverKey);
        ...
        const url = `<?= base_url('safety/driver-maintenance/get-contacts') ?>?driver_key=${driverKey}`;
    ...
    function editContact(contactKey) {
        ...
        fetch(`<?= base_url('safety/driver-maintenance/get-contacts') ?>?driver_key=${driverKey}`)
            .then(response => response.json())
            .then(data => {
    ...
    function loadContactFunctionOptions() {
        return fetch('<?= base_url('safety/driver-maintenance/get-contact-function-options') ?>')
            .then(response => response.json())
            .then(data => {
    ...
    function loadDriverComments() {
        // Get the hidden driver_key field from the agent form
        const driverKeyField = document.querySelector('#driver-form input[name="driver_key"]');
        ...
        const driverKey = driverKeyField.value;
        console.log('loadDriverComments called, driverKey:', driverKey);
        ...
        fetch('<?= base_url('safety/driver-maintenance/get-comments') ?>?driver_key=' + driverKey)
            .then(response => response.json())
            .then(data => {
    ...
    function showCommentModal() {
        const driverKeyField = document.querySelector('#driver-form input[name="driver_key"]');
        const driverKey = driverKeyField ? driverKeyField.value : '0';
